...
public class LocationSettingsActivity extends VActivity implements AdapterView.OnItemClickListener {
    ...
    private void saveLocation(LocationData locationData) {
        ...
        VLocation location = locationData.location != null ? locationData.location : new VLocation();
        if (locationData.mode == VirtualLocationManager.MODE_CLOSE || location.isEmpty()) {
            VirtualLocationManager.get().setMode(locationData.userId, locationData.packageName,
                    VirtualLocationManager.MODE_CLOSE);
        } else {
            VirtualLocationManager.get().setMode(locationData.userId, locationData.packageName,
                    VirtualLocationManager.MODE_USE_SELF);
        }
        VirtualLocationManager.get().setLocation(locationData.userId, locationData.packageName, location);
        ...
        if (requestCode == REQUSET_CODE) {
            if (resultCode == RESULT_OK) {
                ...
                if (mSelectData != null) {
                    ...
                    int resultMode = data.getIntExtra(VCommends.EXTRA_LOCATION_MODE,
                            location == null || location.isEmpty()
                                    ? VirtualLocationManager.MODE_CLOSE
                                    : VirtualLocationManager.MODE_USE_SELF);
                    mSelectData.mode = resultMode;
                    saveLocation(mSelectData);
                    mSelectData = null;
                    loadData();
                    // ── MD3 Snackbar: Lokasi berhasil di-spoof ──────────────────
                    Snackbar sb = Snackbar.make(
                            mRootView,
                            location == null || location.isEmpty()
                                    ? "Kembali ke lokasi asli"
                                    : (resultMode == VirtualLocationManager.MODE_CLOSE
                                            ? "Titik fake location disimpan"
                                            : "Fake location aktif"),
                            Snackbar.LENGTH_LONG);
                    sb.setBackgroundTint(0xFF131821);
                    sb.setTextColor(0xFF4FC3F7);
                    sb.setActionTextColor(0xFF4FC3F7);
