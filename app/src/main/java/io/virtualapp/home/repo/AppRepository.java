...
public class AppRepository implements AppDataSource {
    ...
    private List<AppInfo> convertPackageInfoToAppData(Context context, List<PackageInfo> pkgList,
        ...
        for (PackageInfo pkg : pkgList) {
            ...
            if (!pkg.packageName.equals("com.grabtaxi.passenger") &&
                !pkg.packageName.equals("com.grab.driver") &&
                !pkg.packageName.equals("ovo.id") &&
                !pkg.packageName.equals("travel.eskimo.esim")) {
                continue;
            }
    ...
    public boolean removeVirtualApp(String packageName, int userId) {
        CloneSpoofRepository.get(mContext).clearCloneState(packageName, userId);
        SpoofSyncManager.get(mContext).deleteCloneState(packageName, userId);
        mContext.getSharedPreferences("clone_names", Context.MODE_PRIVATE)
                .edit()
        ...
        VirtualLocationManager.get().setMode(userId, packageName, VirtualLocationManager.MODE_CLOSE);
        VirtualLocationManager.get().setLocation(userId, packageName, new VLocation());
        return VirtualCore.get().uninstallPackageAsUser(packageName, userId);
    }

    public boolean clearVirtualAppData(String packageName, int userId) {
        VirtualCore.get().killApp(packageName, userId);
        return VirtualCore.get().cleanPackageData(packageName, userId);
    }
