...
class PackagePersistenceLayer extends PersistenceLayer {
    ...
    public void readPersistenceData(Parcel p, int version) {
        ...
        while (count-- > 0) {
            ...
            if (!mService.loadPackage(setting)) {
                com.lody.virtual.helper.utils.VLog.w("PackagePersistence",
                        "Preserving persisted entry after load failure: %s", setting.packageName);
            }
        }
    }
    ...
    public void onPersistenceFileDamage() {
        // Replace factory reset with non-destructive behavior
        // Prefer recovery over deletion to preserve app data
        com.lody.virtual.helper.utils.VLog.e("PackagePersistence", "Persistence file damage detected. Avoiding factory reset to preserve user data.");
        // We do NOT call VAppManagerService.get().restoreFactoryState() here.
        // The read() method in PersistenceLayer will already attempt to restore from backup.
        // If that also fails, we just start with an empty cache but don't wipe everything.
    }
}
