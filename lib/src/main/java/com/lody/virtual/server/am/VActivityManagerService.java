...
public class VActivityManagerService extends IActivityManager.Stub {
    ...
    public static void systemReady() {
        ...
        ReceiverCompat.registerReceiver(VirtualCore.get().getContext(), new BroadcastReceiver() {
            @Override
...
//                    finish all
        ...
        }, new IntentFilter(Intent.ACTION_CONFIGURATION_CHANGED), true);
    }
...
//        }
    ...
    ProcessRecord startProcessIfNeedLocked(String processName, int userId, String packageName, int vpid, int callingUid, @VActivityManager.ProcessStartType int type) {
        ...
        synchronized (mProcessLock) {
            ...
            int attemptCount = vpid == -1 ? StubManifest.STUB_COUNT : 1;
            for (int attempt = 0; attempt < attemptCount; attempt++) {
                int candidateVpid = vpid;
                if (candidateVpid == -1) {
                    candidateVpid = queryFreeStubProcess(is64bit);
                }
                if (candidateVpid == -1) {
                    VLog.e(TAG, "Unable to query free stub for : " + processName);
                    app = null;
                ...
                app = new ProcessRecord(info, processName, vuid, candidateVpid, callingUid, is64bit);
                mProcessNames.put(app.processName, app.vuid, app);
                mPidsSelfLocked.add(app);
                if (initProcess(app)) {
                    break;
                }
                VLog.w(TAG, "init process failed for %s using stub p%d", processName, candidateVpid);
                mProcessNames.remove(app.processName, app.vuid);
                mPidsSelfLocked.remove(app);
                app = null;
                if (vpid != -1) {
...
//            }
    ...
    public int queryFreeStubProcess(boolean is64bit) {
        synchronized (mProcessLock) {
            for (int offset = 0; offset < StubManifest.STUB_COUNT; offset++) {
                int vpid = (mNextStubIndex + offset) % StubManifest.STUB_COUNT;
                int N = mPidsSelfLocked.size();
                boolean using = false;
                while (N-- > 0) {
                    ProcessRecord r = mPidsSelfLocked.get(N);
                ...
                mNextStubIndex = (vpid + 1) % StubManifest.STUB_COUNT;
                return vpid;
            }
        }
        return -1;
