        ...
        synchronized (mPackages) {
            ...
            if (p != null) {
                ...
                if (ps.appMode == InstalledAppInfo.MODE_APP_USE_OUTSIDE_APK) {
                    try {
                        ActivityInfo hostActivityInfo = VirtualCore.get().getUnHookPackageManager().getActivityInfo(component, flags | PackageManager.GET_META_DATA);
                        if (hostActivityInfo != null) {
                            ActivityInfo activityInfo = new ActivityInfo(hostActivityInfo);
                            activityInfo.applicationInfo = PackageParserEx.generateApplicationInfo(p, flags, ps.readUserState(userId), userId);
                            if (activityInfo.applicationInfo != null) {
                                ComponentFixer.fixComponentInfo(activityInfo);
                                VLog.w(TAG, "Falling back to host ActivityInfo for outside-apk component %s user=%d", component, userId);
                                return activityInfo;
                            }
                        }
                    } catch (PackageManager.NameNotFoundException ignored) {
                        // Host package manager does not know this component.
                    }
                }
            }
        }
    ...
    public ServiceInfo getServiceInfo(ComponentName component, int flags, int userId) {
        ...
        synchronized (mPackages) {
            ...
            if (p != null) {
                ...
                if (ps.appMode == InstalledAppInfo.MODE_APP_USE_OUTSIDE_APK) {
                    try {
                        ServiceInfo hostInfo = VirtualCore.get().getUnHookPackageManager()
                                .getServiceInfo(component, flags);
                        if (hostInfo != null) {
                            ServiceInfo serviceInfo = new ServiceInfo(hostInfo);
                            serviceInfo.applicationInfo = PackageParserEx.generateApplicationInfo(
                                    p, flags, ps.readUserState(userId), userId);
                            ComponentFixer.fixComponentInfo(serviceInfo);
                            VLog.w(TAG, "Falling back to host ServiceInfo for outside-apk component %s user=%d",
                                    component, userId);
                            return serviceInfo;
                        }
                    } catch (PackageManager.NameNotFoundException e) {
                        VLog.w(TAG, "Host ServiceInfo missing for outside-apk component %s user=%d",
                                component, userId);
                    }
                }
            }
