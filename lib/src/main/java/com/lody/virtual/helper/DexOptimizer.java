...
public class DexOptimizer {
    ...
    public static void optimizeDex(String dexFilePath, String optFilePath) throws IOException {
        VLog.i("DexOptimizer", "Optimizing DEX: %s -> %s", dexFilePath, optFilePath);
        ...
        if (VirtualRuntime.isArt()) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && Build.VERSION.SDK_INT <= Build.VERSION_CODES.N_MR1) {
                try {
                    ...
                    VLog.w("DexOptimizer", "interpretDex2Oat failed (continuing): %s", e.getMessage());
                    // Continue to try DexFile.loadDex
                }
            }
        }
        ...
        try {
            ...
            VLog.i("DexOptimizer", "DEX optimization successful: %s", optFilePath);
        } catch (IOException e) {
            VLog.e("DexOptimizer", "Failed to load DEX: %s - %s", dexFilePath, e.getMessage());
            throw e;
        }
    }
