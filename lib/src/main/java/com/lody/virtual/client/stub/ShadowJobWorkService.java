                ...
                synchronized (mJobSessions) {
                    ...
                    if(VActivityManager.get().isAppRunning(key.packageName, userId, false)) {
                        ...
                        try {
                            ...
                            ServiceInfo serviceInfo = com.lody.virtual.client.core.VirtualCore.get().resolveServiceInfo(service, userId);
                            if (serviceInfo != null) {
                                Intent proxyIntent = VActivityManager.get().bindService(userId, service, serviceInfo,
                                        session.asBinder(), Context.BIND_AUTO_CREATE | Context.BIND_NOT_FOREGROUND);
                                if (proxyIntent != null) {
                                    bound = bindService(proxyIntent, session, Context.BIND_AUTO_CREATE | Context.BIND_NOT_FOREGROUND);
                                }
                            } else {
