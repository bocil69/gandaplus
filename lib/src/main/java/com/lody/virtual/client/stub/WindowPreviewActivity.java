...
public class WindowPreviewActivity extends Activity {
    ...
    private final Runnable mFinishRunnable = new Runnable() {
        ...
        public void run() {
            if (!isFinishing()) {
                VLog.d(TAG, "preview::timeout_finish");
                finish();
            }
        }
    };

    // Polls isLaunched() every second so the preview dismisses as soon as the
    // real activity has drawn its first frame — rather than waiting for the hard timeout.
    private final Runnable mCheckLaunchedRunnable = new Runnable() {
        @Override
        public void run() {
            if (isFinishing()) return;
            if (checkLaunched("poll")) {
                VLog.d(TAG, "preview::poll_launched_finish");
                finish();
                return;
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (com.lody.virtual.helper.compat.BuildCompat.isT()) {
            // Android 13+ / HyperOS: some apps (e.g. Grab) take 30s+ to start.
            // Use a 45s hard timeout as a safety net, and poll every 1s so we
            // dismiss immediately once the real activity becomes visible.
            getWindow().getDecorView().postDelayed(mFinishRunnable, 45000L);
            getWindow().getDecorView().postDelayed(mCheckLaunchedRunnable, 1000L);
        }
...
//                TextView textTip = findViewById(R.id.tv_tip);
    ...
    protected void onDestroy() {
        ...
        getWindow().getDecorView().removeCallbacks(mCheckLaunchedRunnable);
        super.onDestroy();
    }
}
