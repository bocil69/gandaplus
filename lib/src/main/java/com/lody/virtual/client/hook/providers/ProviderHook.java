...
public class ProviderHook implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object... args) throws Throwable {
        Object[] sanitizedArgs = args;
        try {
            sanitizedArgs = sanitizeArgs(method, args);
        } catch (Throwable e) {
            e.printStackTrace();
        }
        MethodBox methodBox = new MethodBox(method, mBase, sanitizedArgs);
        ...
        try {
            ...
            if ("call".equals(name)) {
                ...
                String methodName = (String) sanitizedArgs[start];
                String arg = (String) sanitizedArgs[start + 1];
                Bundle extras = (Bundle) sanitizedArgs[start + 2];
                return call(methodBox, methodName, arg, extras);
            } else if ("insert".equals(name)) {
                Uri url = (Uri) sanitizedArgs[start];
                ContentValues initialValues = (ContentValues) sanitizedArgs[start + 1];
                return insert(methodBox, url, initialValues);
            } else if ("getType".equals(name)) {
                return getType(methodBox, (Uri) sanitizedArgs[0]);
            } else if ("delete".equals(name)) {
                Uri url = (Uri) sanitizedArgs[start];
                String selection = (String) sanitizedArgs[start + 1];
                String[] selectionArgs = (String[]) sanitizedArgs[start + 2];
                ...
                Uri url = (Uri) sanitizedArgs[start];
                ContentValues values = (ContentValues) sanitizedArgs[start + 1];
                String selection = (String) sanitizedArgs[start + 2];
                String[] selectionArgs = (String[]) sanitizedArgs[start + 3];
                return update(methodBox, url, values, selection, selectionArgs);
            } else if ("openFile".equals(name)) {
                Uri url = (Uri) sanitizedArgs[start];
                String mode = (String) sanitizedArgs[start + 1];
                ...
                Uri url = (Uri) sanitizedArgs[start];
                String[] projection = (String[]) sanitizedArgs[start + 1];
                String selection = null;
                String[] selectionArgs = null;
                String sortOrder = null;
                Bundle queryArgs = null;
                if (BuildCompat.isOreo()) {
                    queryArgs = (Bundle) sanitizedArgs[start + 2];
                    if (queryArgs != null) {
                    ...
                    selection = (String) sanitizedArgs[start + 2];
                    selectionArgs = (String[]) sanitizedArgs[start + 3];
                    sortOrder = (String) sanitizedArgs[start + 4];
                }
                return query(methodBox, url, projection, selection, selectionArgs, sortOrder, queryArgs);
            }
            return methodBox.call();
        } catch (Throwable e) {
            VLog.w("ProviderHook", "call: %s (%s) with error", method.getName(), Arrays.toString(sanitizedArgs));
    ...
    private Object[] sanitizeArgs(Method method, Object... args) {
        ...
        processArgs(method, sanitizedArgs);
        for (int i = 0; i < sanitizedArgs.length; i++) {
            ...
            if (isAttributionSource(arg)) {
                VLog.w("ProviderHook", "Replacing AttributionSource argument for %s to avoid uid mismatch in virtual process", method.getName());
                sanitizedArgs[i] = createSafeAttributionSource();
                continue;
            }
            if (arg instanceof Bundle) {
                sanitizedArgs[i] = sanitizeBundle(method, (Bundle) arg);
            }
        }
    ...
    private Bundle sanitizeBundle(Method method, Bundle source) {
        ...
        for (String key : sanitized.keySet()) {
            ...
            if (isAttributionSource(value)) {
                VLog.w("ProviderHook", "Replacing AttributionSource bundle entry %s for %s", key, method.getName());
    ...
    private AttributionSource createSafeAttributionSource() {
        ...
        try {
            ...
            VLog.w("ProviderHook", "Unable to build safe AttributionSource for uid=%d pkg=%s: %s", uid, packageName, e.getMessage());
            return null;
        }
    }

    private boolean isAttributionSource(Object arg) {
