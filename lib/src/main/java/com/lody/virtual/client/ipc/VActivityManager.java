...
public class VActivityManager {
    ...
    private ActivityInfo resolveActivityInfoCompat(Intent intent, int userId) {
        ActivityInfo info = VirtualCore.get().resolveActivityInfo(intent, userId);
        if (info == null && com.lody.virtual.helper.compat.BuildCompat.isT() && intent != null && intent.getComponent() != null) {
            VLog.w(TAG, "resolveActivityInfoCompat fallback to component for user=%d component=%s", userId, intent.getComponent());
            info = VirtualCore.get().resolveActivityInfo(intent.getComponent(), userId);
        }
        return info;
    }

    private boolean isStartActivityResultSuccessful(int result) {
        return result == ActivityManagerCompat.START_SUCCESS
                || result == ActivityManagerCompat.START_TASK_TO_FRONT;
    }

    private boolean startActivityChecked(Intent intent, int userId, String packageName, String stage) {
        int result = startActivity(intent, userId);
        if (!isStartActivityResultSuccessful(result)) {
            VLog.w(TAG, "launchApp startActivity failed at %s for %s user=%d result=%d intent=%s",
                    stage, packageName, userId, result, intent);
            return false;
        }
        VLog.i(TAG, "launchApp startActivity succeeded at %s for %s user=%d result=%d",
                stage, packageName, userId, result);
        return true;
    }

    public int startActivity(Intent intent, ActivityInfo info, IBinder resultTo, Bundle options, String resultWho, int requestCode, int userId) {
        if (info == null) {
            info = resolveActivityInfoCompat(intent, userId);
            if (info == null) {
    ...
    public int startActivity(Intent intent, int userId) {
        ...
        ActivityInfo info = resolveActivityInfoCompat(intent, userId);
        if (info == null) {
            return ActivityManagerCompat.START_INTENT_NOT_RESOLVED;
        }
    ...
    public boolean launchApp(final int userId, final String packageName, boolean preview) {
        ...
        if (intent == null) {
            VLog.w(TAG, "launchApp failed to resolve launch intent for %s user=%d", packageName, userId);
            return false;
        }
        final ActivityInfo info = resolveActivityInfoCompat(intent, userId);
        if (info == null) {
            VLog.w(TAG, "launchApp failed to resolve activity info for %s user=%d intent=%s", packageName, userId, intent);
            return false;
        }
        //1.va的进程初始化500ms
        ...
        if (!preview || VActivityManager.get().isAppRunning(info.packageName, userId, true)) {
            ...
            return startActivityChecked(intent, userId, packageName, preview ? "running_with_preview" : "running_no_preview");
        } else {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (clientConfig != null) {
                        if (!startActivityChecked(intent, userId, packageName, preview ? "post_init_preview" : "post_init_no_preview")) {
                            VLog.w(TAG, "launchApp post-init startActivity aborted for %s user=%d", packageName, userId);
                        }
                        //VActivityManager#startActivity启动速度比WindowPreviewActivity快
                    } else {
                        VLog.w(TAG, "initProcess returned null for %s user=%d process=%s", packageName, userId, processName);
                    }
                }
            }).start();
        }
