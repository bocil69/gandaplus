...
public final class AppInstrumentation extends InstrumentationDelegate implements IInjector {
    ...
    public void callActivityOnCreate(Activity activity, Bundle icicle) {
        ...
        ensureEskimoFacebookSdkInitializedForLogin(activity);
    ...
    private void ensureEskimoFacebookSdkInitializedForLogin(Activity activity) {
        if (activity == null) {
            return;
        }
        String currentPackage = VClient.get().getCurrentPackage();
        if (!"travel.eskimo.esim".equals(currentPackage)) {
            return;
        }
        if (!"com.hotspot.travel.hotspot.activity.LoginActivity".equals(activity.getClass().getName())) {
            return;
        }
        ClassLoader classLoader = activity.getClassLoader();
        triggerEskimoFacebookInitProvider(activity, classLoader, "travel.eskimo.esim");
        try {
            ...
            if (isFacebookSdkInitialized(facebookSdkClass)) {
                VLog.i(TAG, "Eskimo FacebookSdk already initialized before LoginActivity");
                return;
            }
            ...
            invokeContextInitializerIfPresent(facebookSdkClass, activity.getApplication());
            invokeNoArgIfPresent(facebookSdkClass, "fullyInitialize");
            if (isFacebookSdkInitialized(facebookSdkClass)) {
                VLog.i(TAG, "Eskimo FacebookSdk initialized via LoginActivity fallback hook");
            } else {
                VLog.w(TAG, "Eskimo LoginActivity fallback ran but FacebookSdk still reports uninitialized");
            }
        } catch (ClassNotFoundException e) {
            VLog.w(TAG, "Eskimo LoginActivity fallback could not find FacebookSdk");
        } catch (Throwable e) {
            VLog.w(TAG, "Eskimo LoginActivity fallback failed: %s", e.getMessage());
        }
    }
    ...
    private void triggerEskimoFacebookInitProvider(Context context, ClassLoader classLoader, String packageName) {
        ...
        try {
            ...
            if (!(provider instanceof ContentProvider)) {
                VLog.w(TAG, "Eskimo FacebookInitProvider class is not a ContentProvider before LoginActivity");
                return;
            }
            ...
            VLog.i(TAG, "Eskimo FacebookInitProvider bootstrapped in-process before LoginActivity");
        } catch (Throwable e) {
            VLog.w(TAG, "Eskimo FacebookInitProvider bootstrap before LoginActivity failed: %s", e.getMessage());
        }
    }
