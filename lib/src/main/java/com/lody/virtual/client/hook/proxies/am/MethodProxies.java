...
//            }
    ...
    static class GetRecentTasks extends MethodProxy {
        ...
        public Object call(Object who, Method method, Object... args) throws Throwable {
            ...
            List<ActivityManager.RecentTaskInfo> infos = slice ? (List<ActivityManager.RecentTaskInfo>) ParceledListSlice.getList.call(_infos) : (List<ActivityManager.RecentTaskInfo>) _infos;
...
//            String resolvedType = (String) args[1];
    ...
    static class GetIntentSender extends MethodProxy {
        ...
        private int findCreatorIndex(Object[] args) {
            if (args == null) {
                return -1;
            }
            for (int i = 1; i < args.length; i++) {
                Object arg = args[i];
                if (arg instanceof String && VirtualCore.get().isAppInstalled((String) arg)) {
                    return i;
                }
            }
            return -1;
        ...
        private int findIntentArrayIndex(Object[] args) {
            return ArrayUtils.indexOfObject(args, Intent[].class, 0);
        }

        private int findResolvedTypesIndex(Object[] args, int intentIndex) {
            if (intentIndex == -1) {
                return -1;
            }
            return ArrayUtils.indexOfObject(args, String[].class, intentIndex + 1);
        }

        private int findFlagsIndex(Object[] args, int start) {
            if (args == null) {
                return -1;
            }
            for (int i = Math.max(0, start); i < args.length; i++) {
        ...
        @Override
        public Object call(Object who, Method method, Object... args) throws Throwable {
            int creatorIndex = findCreatorIndex(args);
            String creator = creatorIndex != -1 ? (String) args[creatorIndex] : getHostPkg();
            if (creatorIndex != -1) {
                args[creatorIndex] = getHostPkg();
            } else {
                MethodParameterUtils.replaceFirstAppPkg(args);
            }
            // force userId to 0
            ...
            int intentIndex = findIntentArrayIndex(args);
            if (intentIndex == -1) {
                return method.invoke(who, args);
            }
            int resolvedTypesIndex = findResolvedTypesIndex(args, intentIndex);
            String[] resolvedTypes = resolvedTypesIndex != -1 ? (String[]) args[resolvedTypesIndex] : null;
            int flagsIndex = findFlagsIndex(args, resolvedTypesIndex != -1 ? resolvedTypesIndex + 1 : intentIndex + 1);
            int type = (int) args[0];
            int flags = flagsIndex != -1 ? (int) args[flagsIndex] : 0;
            Intent[] intents = (Intent[]) args[intentIndex];
            ...
            if (intents.length > 0) {
                ...
                args[intentIndex] = new Intent[]{targetIntent};
                if (resolvedTypesIndex != -1) {
...
//            }
            ...
            if (pkg != null && !isAppPkg(pkg)) {
                // Android 11+ (API 30+): IActivityTaskManager.startActivity gained a
                // callerFeatureId param BEFORE the intent arg, shifting the layout to:
                //   args[0]=caller, args[1]=callerPackage, args[2]=callerFeatureId, args[3]=intent
                // So intentIndex == 3 and callerPackage is at intentIndex-2, not intentIndex-1.
                // On Android 13, AMS validates callerPackage belongs to the calling UID, so
                // when the guest app fires an intent to an external package we must rewrite it.
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    // callerPackage is always at args[1] on Android 11+
                    args[1] = getHostPkg();
                }
                return method.invoke(who, args);
            }
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
                // Keep legacy behavior for pre-Android 13 devices.
                // Android 13+ validates callerPackage strictly and the modern signature keeps
                // callerPackage at args[1], with callerFeatureId inserted before the intent.
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    args[1] = getHostPkg();
                } else {
            ...
            if (activityInfo == null) {
                ...
                // Force deep-link intercept for specific domains/schemes to avoid host leak
                if (Intent.ACTION_VIEW.equals(intent.getAction()) && intent.getData() != null) {
                    Uri data = intent.getData();
                    String scheme = data.getScheme();
                    String host = data.getHost();
                    String forcedTarget = null;
                    if ("ovo".equals(scheme) || (host != null && host.contains("ovo"))) {
                        forcedTarget = "ovo.id";
                    } else if ("grab".equals(scheme) || (host != null && host.contains("grab"))) {
                        if (VirtualCore.get().isAppInstalled("com.grabtaxi.passenger")) {
                            forcedTarget = "com.grabtaxi.passenger";
                    ...
                    if (forcedTarget != null && VirtualCore.get().isAppInstalled(forcedTarget)) {
                        Intent testIntent = new Intent(intent);
                        testIntent.setPackage(forcedTarget);
                        if ("ovo.id".equals(forcedTarget) || VirtualCore.get().resolveActivityInfo(testIntent, userId) != null) {
                            Intent pickerIntent = VirtualCore.getConfig().getDeepLinkPickerIntent(intent, forcedTarget);
                            if (pickerIntent != null) {
                                getHostContext().startActivity(pickerIntent);
                                return 0;
                            }
                        }
                    }
                ...
                if (Intent.ACTION_VIEW.equals(intent.getAction()) && intent.getComponent() == null && intent.getPackage() == null) {
                    ResolveInfo hostResolve = VirtualCore.get().getUnHookPackageManager().resolveActivity(intent, PackageManager.MATCH_DEFAULT_ONLY);
                    if (hostResolve != null && hostResolve.activityInfo != null) {
                        String targetPkg = hostResolve.activityInfo.packageName;
                        if (VirtualCore.get().isAppInstalled(targetPkg)) {
...
//            boolean doRebind = (boolean) args[2];
    ...
    static class BindServiceWithFeature extends BindService {

        @Override
        public String getMethodName() {
            return "bindServiceWithFeature";
        }
    }
    ...
    static class BindService extends MethodProxy {
        ...
        private int findFlagsIndex(Object[] args, int connIndex) {
            for (int i = connIndex + 1; i < args.length; i++) {
                Object arg = args[i];
        ...
        @Override
        public Object call(Object who, Method method, Object... args) throws Throwable {
            int intentIndex = ArrayUtils.indexOfObject(args, Intent.class, 0);
            int connIndex = ArrayUtils.indexOfObject(args, IServiceConnection.class, 0);
            if (intentIndex < 0 || connIndex < 0) {
                return method.invoke(who, args);
            }
            Intent service = (Intent) args[intentIndex];
            int resolvedTypeIndex = findResolvedTypeIndex(args, intentIndex);
            String resolvedType = resolvedTypeIndex >= 0 ? (String) args[resolvedTypeIndex] : null;
            IServiceConnection conn = (IServiceConnection) args[connIndex];
            int flagsIndex = findFlagsIndex(args, connIndex);
            int flags = flagsIndex >= 0 && args[flagsIndex] instanceof Number
            ...
            if (serviceInfo != null) {
                ...
                args[connIndex] = delegate;
                if (flagsIndex >= 0) {
                    if (args[flagsIndex] instanceof Long) {
                        args[flagsIndex] = (long) Context.BIND_AUTO_CREATE;
                    } else {
                        args[flagsIndex] = Context.BIND_AUTO_CREATE;
                    }
                }
                return method.invoke(who, args);
            }
    ...
    static class StartForegroundService extends StartService {
    ...
    static class StartServiceWithFeature extends StartService {

        @Override
        public String getMethodName() {
            return "startServiceWithFeature";
        }
    }
...
//            VLog.d("VActivityManager", "getRunningAppProcesses:%s", appProcessList);
    ...
    /**
     * Android 13+ renamed registerReceiver to registerReceiverWithFeature and added a
     * featureId + receiverId parameter. AMS validates that callerPackage matches the real
     * UID of the calling process, so we must rewrite it to the host package just like the
     * original RegisterReceiver proxy does.
     *
     * Signature (Android 13):
     *   registerReceiverWithFeature(IApplicationThread caller,
     *       String callerPackage,      // arg[1]  ← rewrite to host pkg
     *       String callerFeatureId,    // arg[2]
     *       String receiverId,         // arg[3]
     *       IIntentReceiver receiver,  // arg[4]
     *       IntentFilter filter,       // arg[5]
     *       String requiredPermission, // arg[6]  ← null-out
     *       int userId,               // arg[7]  ← replace with real user id
     *       int flags)                // arg[8]
     */
    @TargetApi(Build.VERSION_CODES.TIRAMISU)
    static class RegisterReceiverWithFeature extends RegisterReceiver {
        private static final int IDX_W_IIntentReceiver = 4;
        private static final int IDX_W_IntentFilter    = 5;
        private static final int IDX_W_RequiredPerm    = 6;
        ...
        public Object call(Object who, Method method, Object... args) throws Throwable {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) {
                // Shouldn't be called on older APIs, but be safe.
                return method.invoke(who, args);
            }
            // Rewrite callerPackage (arg[1]) to host package.
            MethodParameterUtils.replaceFirstAppPkg(args);
            // Replace userId (arg[7]) with the real user id.
            replaceFirstUserId(args);
            // Null out required permission so virtual apps don't get blocked.
            if (args.length > IDX_W_RequiredPerm) {
                args[IDX_W_RequiredPerm] = null;
            }
            // Protect the IntentFilter (arg[5]).
            if (args.length > IDX_W_IntentFilter) {
                IntentFilter filter = (IntentFilter) args[IDX_W_IntentFilter];
                if (filter != null) {
                    filter = new IntentFilter(filter);
                    SpecialComponentList.protectIntentFilter(filter);
                    args[IDX_W_IntentFilter] = filter;
                }
            }
            return method.invoke(who, args);
        }
        ...
        @Override
        public boolean isEnable() {
            return isAppProcess() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU;
        }
    }
