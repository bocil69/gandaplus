...
public class NativeEngine {
    ...
    static {
        try {
            // Initialize ShadowHook with proper error handling
            // Delay ShadowHook init to avoid interfering with DEX optimization during app installation
            if (!com.lody.virtual.client.core.VirtualCore.get().isMainProcess()) {
                try {
                    com.bytedance.shadowhook.ShadowHook.init(new com.bytedance.shadowhook.ShadowHook.ConfigBuilder()
                        .setMode(com.bytedance.shadowhook.ShadowHook.Mode.UNIQUE)
                        .build());
                } catch (Throwable e) {
                    VLog.w(TAG, "ShadowHook init failed (non-fatal): " + e.getMessage());
                }
            }
        } catch (Throwable e) {
            VLog.w(TAG, "ShadowHook init check failed: " + e.getMessage());
        }
        ...
        try {
            ...
            VLog.e(TAG, "Failed to load native library: " + VLog.getStackTraceString(e));
        }
    }
    ...
    private static boolean shouldSkipNativeEngineHooks() {
        return VirtualRuntime.is64bit() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU;
    }

    public static void bypassHiddenAPI(){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            try {
                // Android 13+ (Tiramisu) requires special handling for HyperOS/MIUI
                // Use deep reflection to avoid triggering security exceptions
                ...
                // Universal bypass for all hidden APIs (Android 9 to 14+)
                // Using "L" wildcard exempts all hidden APIs
                setHiddenApiExemptions.invoke(runtime, new Object[]{
                        new String[]{"L"}
                });
                
                VLog.i(TAG, "Hidden API bypass successful for API " + Build.VERSION.SDK_INT);
            } catch (Throwable e) {
                // Log but don't crash - app can still work with limited functionality
                VLog.w(TAG, "Hidden API bypass failed (non-fatal): " + e.getMessage());
                // Try alternative method for Android 13+ HyperOS
                tryBypassHiddenAPIAlternative();
            }
        }
    }
    
    /**
     * Alternative bypass method for Android 13+ devices with strict security policies
     * Uses individual class exemptions as fallback
     */
    private static void tryBypassHiddenAPIAlternative() {
        ...
        try {
            ...
            // Exempt common restricted packages individually
            String[] exemptions = new String[]{
                "Landroid/app/",
                "Landroid/content/",
                "Landroid/os/",
                "Landroid/view/",
            ...
            VLog.i(TAG, "Alternative Hidden API bypass successful");
        } catch (Throwable e2) {
            VLog.e(TAG, "Alternative bypass also failed: " + e2.getMessage());
        }
    }
