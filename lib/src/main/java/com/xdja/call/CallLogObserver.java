...
//                }
    ...
    private static void doObserve() {
        CallLogObserver observer = getInstance();
        if (!observer.hasPermission(Manifest.permission.READ_CALL_LOG)) {
            Log.w(TAG, "Skip observing call log: missing READ_CALL_LOG permission.");
            return;
        }
        try {
            observer.getContext().getContentResolver().registerContentObserver(CallLog.Calls.CONTENT_URI, true, observer);
        } catch (SecurityException e) {
            Log.w(TAG, "Unable to register call log observer.", e);
        } catch (Throwable e) {
            Log.w(TAG, "Unexpected failure while registering call log observer.", e);
        }
    }
            ...
            switch (state) {
                ...
                case TelephonyManager.CALL_STATE_OFFHOOK:
                    if (!DIALING) {
                        ...
                        if (VirtualCore.get().getHostPkg().equals(defaultDialerPackage)) {
    ...
    private void listenPhoneState(@NonNull Context context) {
        if (!hasPermission(Manifest.permission.READ_PHONE_STATE)) {
            Log.w(TAG, "Skip listening phone state: missing READ_PHONE_STATE permission.");
            return;
        }
        TelephonyManager telephonyManager = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);
        if (telephonyManager == null) {
            Log.w(TAG, "Skip listening phone state: TelephonyManager unavailable.");
            return;
        }
        try {
            telephonyManager.listen(phoneStateListener, PhoneStateListener.LISTEN_CALL_STATE);
        } catch (SecurityException e) {
            Log.w(TAG, "Unable to listen for call state changes.", e);
        } catch (Throwable e) {
            Log.w(TAG, "Unexpected failure while listening for call state changes.", e);
        }
    }
    ...
    public static void unObserve() {
        ...
        if (telephonyManager == null) {
            Log.w(TAG, "Skip unobserving call log: TelephonyManager unavailable.");
            return;
        }
    ...
    private static void doUnObserve() {
        try {
            ...
            Log.w(TAG, "Call log observer was not registered.", e);
        } catch (Throwable e) {
            Log.w(TAG, "Unexpected failure while unregistering call log observer.", e);
        }
    }
}
