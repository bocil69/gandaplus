...
public class VAppPermissionManager {
    ...
    private final android.util.ArrayMap<String, PermissionQueryCache> mPermissionCache = new android.util.ArrayMap<>();

    private static final class PermissionQueryCache {
        final boolean enabled;
        final long updatedAt;

        PermissionQueryCache(boolean enabled, long updatedAt) {
    ...
    private IAppPermission requireService() {
        return getService();
    }
    ...
    public boolean isSupportPermission(String permissionName) {
        ...
        try {
            IAppPermission service = requireService();
            return service != null && service.isSupportPermission(permissionName);
    ...
    public void setAppPermission(String packageName, String appPermissionName, boolean isPermissionOpen) {
        ...
        try {
            ...
            service.setAppPermission(packageName, appPermissionName, isPermissionOpen);
            removePermissionCache(packageName, appPermissionName);
        } catch (RemoteException e) {
            e.printStackTrace();
            VirtualRuntime.crash(e);
        }
    ...
    public boolean getAppPermissionEnable(String packageName, String appPermissionName) {
        if (TextUtils.isEmpty(packageName) || TextUtils.isEmpty(appPermissionName)) {
            return false;
        }
        PermissionQueryCache cache = readPermissionCache(packageName, appPermissionName);
        if (cache != null) {
        ...
        try {
            ...
            boolean appPermissionEnable = service.getAppPermissionEnable(packageName, appPermissionName);
            cachePermission(packageName, appPermissionName, appPermissionEnable);
    ...
    public void registerCallback(IAppPermissionCallback iAppPermissionCallback) {
        ...
        try {
            ...
            service.registerCallback(iAppPermissionCallback);
        } catch (RemoteException e) {
            e.printStackTrace();
            VirtualRuntime.crash(e);
        }
    ...
    public void interceptorTriggerCallback(String appPackageName, String permissionName) {
        ...
        try {
            ...
            service.interceptorTriggerCallback(appPackageName, permissionName);
        } catch (RemoteException e) {
            e.printStackTrace();
            VirtualRuntime.crash(e);
        }
    ...
    public void cacheClipData(ClipData clipData) {
        ...
        try {
            ...
            service.cacheClipData(clipData);
        } catch (RemoteException e) {
            e.printStackTrace();
            VirtualRuntime.crash(e);
        }
    ...
    public void cachePrimaryClipChangedListener(IOnPrimaryClipChangedListener listener) {
        ...
        try {
            ...
            service.cachePrimaryClipChangedListener(listener);
        } catch (RemoteException e) {
            e.printStackTrace();
            VirtualRuntime.crash(e);
        }
    ...
    public void callPrimaryClipChangedListener() {
        ...
        try {
            ...
            service.callPrimaryClipChangedListener();
        } catch (RemoteException e) {
            e.printStackTrace();
            VirtualRuntime.crash(e);
        }
    ...
    public void removePrimaryClipChangedListener() {
        ...
        try {
            ...
            service.removePrimaryClipChangedListener();
        } catch (RemoteException e) {
            e.printStackTrace();
            VirtualRuntime.crash(e);
        }
    ...
    public void setThirdAppInstallationEnable(boolean isEnable) {
        ...
        try {
            ...
            service.setThirdAppInstallationEnable(isEnable);
        } catch (RemoteException e) {
            e.printStackTrace();
            VirtualRuntime.crash(e);
        }
    ...
    public String[] getEncryptConfig() {
        try {
            ...
            return service.getEncryptConfig();
        } catch (RemoteException e) {
            e.printStackTrace();
            VirtualRuntime.crash(e);
            return  null;
    ...
    public boolean getThirdAppInstallationEnable() {
        try {
            ...
            boolean thirdAppInstallationEnable = service.getThirdAppInstallationEnable();
    ...
    private PermissionQueryCache readPermissionCache(String packageName, String appPermissionName) {
        String cacheKey = buildPermissionCacheKey(packageName, appPermissionName);
        synchronized (mPermissionCache) {
            PermissionQueryCache cache = mPermissionCache.get(cacheKey);
            if (cache == null) {
                return null;
            }
            if (SystemClock.elapsedRealtime() - cache.updatedAt > CACHE_TTL_MS) {
                mPermissionCache.remove(cacheKey);
                return null;
            }
            return cache;
        }
    ...
    private void cachePermission(String packageName, String appPermissionName, boolean enabled) {
        String cacheKey = buildPermissionCacheKey(packageName, appPermissionName);
        synchronized (mPermissionCache) {
            mPermissionCache.put(cacheKey, new PermissionQueryCache(enabled, SystemClock.elapsedRealtime()));
        }
    }

    private void removePermissionCache(String packageName, String appPermissionName) {
        if (TextUtils.isEmpty(packageName) || TextUtils.isEmpty(appPermissionName)) {
            return;
        }
        synchronized (mPermissionCache) {
            mPermissionCache.remove(buildPermissionCacheKey(packageName, appPermissionName));
        }
    }

    private String buildPermissionCacheKey(String packageName, String appPermissionName) {
