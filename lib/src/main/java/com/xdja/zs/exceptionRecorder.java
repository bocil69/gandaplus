    ...
    public static String getExceptionRecordPath()
    {
        ...
        if (context != null) {
            try {
                ...
                if (externalDir != null) {
                    return new File(externalDir, SSBEL).getAbsolutePath();
                }
            } catch (Throwable e) {
                VLog.w("exceptionRecorder", "Unable to resolve external exception log dir, falling back to internal storage: %s", e.getMessage());
            }

            File filesDir = context.getFilesDir();
            if (filesDir != null) {
                return new File(filesDir, SSBEL).getAbsolutePath();
            }
        }

        return new File("/data/local/tmp", SSBEL).getAbsolutePath();
    }
