...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *_vm, void *) {
    ...
    // SafekeyManager - guarded: class may be absent when running without xdja security SDK
    jclass VSKMClass = env->FindClass("com/xdja/zs/VSafekeyManager");
    if (env->ExceptionCheck()) {
        env->ExceptionClear(); // Class not found is non-fatal for us
    }
    if (VSKMClass != NULL) {
        vskmClass = (jclass) env->NewGlobalRef(VSKMClass);
        env->DeleteLocalRef(VSKMClass);
    } else {
        vskmClass = NULL; // safekey_jni will guard against this
    }
