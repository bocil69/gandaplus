...
public class RequestBuilder<TranscodeType> extends BaseRequestOptions<RequestBuilder<TranscodeType>> implements Cloneable, ModelTypes<RequestBuilder<TranscodeType>> {
    ...
    public RequestBuilder(@NonNull Glide glide, RequestManager requestManager, Class<TranscodeType> transcodeClass, Context context) {
    ...
    public RequestBuilder<TranscodeType> transition(@NonNull TransitionOptions<?, ? super TranscodeType> transitionOptions) {
        if (isAutoCloneEnabled()) {
            return mo30clone().transition(transitionOptions);
        }
    ...
    public RequestBuilder<TranscodeType> listener(@Nullable RequestListener<TranscodeType> requestListener) {
        if (isAutoCloneEnabled()) {
            return mo30clone().listener(requestListener);
        }
    ...
    public RequestBuilder<TranscodeType> addListener(@Nullable RequestListener<TranscodeType> requestListener) {
        if (isAutoCloneEnabled()) {
            return mo30clone().addListener(requestListener);
        }
    ...
    public RequestBuilder<TranscodeType> error(@Nullable RequestBuilder<TranscodeType> errorBuilder) {
        if (isAutoCloneEnabled()) {
            return mo30clone().error((RequestBuilder) errorBuilder);
        }
    ...
    private RequestBuilder<TranscodeType> cloneWithNullErrorAndThumbnail() {
        return mo30clone().error((RequestBuilder) null).thumbnail((RequestBuilder) null);
    }
    ...
    public RequestBuilder<TranscodeType> thumbnail(@Nullable RequestBuilder<TranscodeType> thumbnailRequest) {
        if (isAutoCloneEnabled()) {
            return mo30clone().thumbnail(thumbnailRequest);
        }
    ...
    public RequestBuilder<TranscodeType> thumbnail(float sizeMultiplier) {
        if (isAutoCloneEnabled()) {
            return mo30clone().thumbnail(sizeMultiplier);
        }
    ...
    private RequestBuilder<TranscodeType> loadGeneric(@Nullable Object model) {
        if (isAutoCloneEnabled()) {
            return mo30clone().loadGeneric(model);
        }
        this.model = model;
    ...
    @CheckResult
    /* renamed from: clone */
    public RequestBuilder<TranscodeType> mo30clone() {
        RequestBuilder<TranscodeType> result = (RequestBuilder) super.mo30clone();
        result.transitionOptions = (TransitionOptions<?, ? super TranscodeType>) result.transitionOptions.m31clone();
        ...
        if (result.thumbnailBuilder != null) {
            result.thumbnailBuilder = result.thumbnailBuilder.mo30clone();
        }
        if (result.errorBuilder != null) {
            result.errorBuilder = result.errorBuilder.mo30clone();
        }
        return result;
    }
    ...
    public ViewTarget<ImageView, TranscodeType> into(@NonNull ImageView view) {
        ...
        if (!requestOptions.isTransformationSet() && requestOptions.isTransformationAllowed() && view.getScaleType() != null) {
            switch (AnonymousClass1.$SwitchMap$android$widget$ImageView$ScaleType[view.getScaleType().ordinal()]) {
                case 1:
                    requestOptions = requestOptions.mo30clone().optionalCenterCrop();
                    break;
                case 2:
                    requestOptions = requestOptions.mo30clone().optionalCenterInside();
                    break;
                case 3:
                case 4:
                case 5:
                    requestOptions = requestOptions.mo30clone().optionalFitCenter();
                    break;
                case 6:
                    requestOptions = requestOptions.mo30clone().optionalCenterInside();
                    break;
    ...
    private Request buildThumbnailRequestRecursive(Object requestLock, Target<TranscodeType> target, RequestListener<TranscodeType> targetListener, @Nullable RequestCoordinator parentCoordinator, TransitionOptions<?, ? super TranscodeType> transitionOptions, Priority priority, int overrideWidth, int overrideHeight, BaseRequestOptions<?> requestOptions, Executor callbackExecutor) {
        ...
        if (this.thumbnailBuilder != null) {
            ...
            BaseRequestOptions<?> thumbnailOptions = requestOptions.mo30clone().sizeMultiplier(this.thumbSizeMultiplier.floatValue());
